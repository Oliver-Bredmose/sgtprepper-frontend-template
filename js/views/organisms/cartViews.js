...
export const cartListView = (data = []) => {
    ...
    data.forEach(item => {
        ...
        const quantity = Div('w-[10%] font-bold')
        ...
        const price = Div('text-right-3 w-[20%]')
...
export const cartTotalView = totalPrice => {
    const totalRow = Div('flex gap-3 border-b border-t border-t py-1 justify-between')

    const textCol = Div('w-[70%] font-bold')
    textCol.innerText = 'Total'

    const totalCol = Div('w-[20%] text-right-3 font-bold')
